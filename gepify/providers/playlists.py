...
@celery_app.task
def create_zip_playlist(playlist, service, format='mp3'):
    ...
    playlist_zip.close()
    cache.set(playlist_cache_key, playlist_zip_filename)
...
def download_playlist(playlist, service, provider='youtube', format='mp3'):
    ...
    playlist_cache_key = '{}_{}_{}'.format(service, playlist['id'], format)
    if cache.get(playlist_cache_key) is not None:
        return
    ...
    if len(download_song_tasks) == 0:
        create_zip_playlist.delay(playlist, service, format)
    else:
        chord(download_song_tasks,
              create_zip_playlist.si(playlist, service, format)).delay()
